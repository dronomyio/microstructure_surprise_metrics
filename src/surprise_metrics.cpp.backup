...
private:
    ...
    void init_cuda() {
        ...
        cudaMalloc(&d_jump_flags_, buffer_size_ * sizeof(bool));
        ...
        cudaMalloc(&d_burst_flags_, buffer_size_ * sizeof(bool));
    ...
    void process_gpu(const std::vector<Trade>& trades) {
        ...
        // Copy data to GPU
        cudaMemcpy(d_returns_, return_buffer_.data(), n_returns * sizeof(float), cudaMemcpyHostToDevice);
        cudaMemcpy(d_timestamps_, timestamp_buffer_.data(), n_trades * sizeof(float), cudaMemcpyHostToDevice);
        cudaMemcpy(d_prices_, price_buffer_.data(), n_trades * sizeof(float), cudaMemcpyHostToDevice);
        ...
        int blocks = (n_returns + cuda::BLOCK_SIZE - 1) / cuda::BLOCK_SIZE;
	// can't call kernels directly from .cpp
        //cuda::bns_kernel<<<blocks, cuda::BLOCK_SIZE>>>(
        // Then in surprise_metrics.cpp call:
        cuda::launch_bns_computation(d_returns_, d_rv_, d_bv_, d_tq_, 
			             d_bns_stats_, n_returns, window_size_, 0);
        ...
        //cuda::lee_mykland_kernel<<<blocks, cuda::BLOCK_SIZE>>>(
        //    d_returns_, d_local_vol, d_lm_stats, d_jump_flags_, 
        //    n_returns, window_size_, jump_threshold_
        //);
	
	// Lee-Mykland computation
	cuda::launch_lee_mykland_computation(
	    d_returns_, d_local_vol, d_lm_stats, d_jump_flags_,
	    n_returns, window_size_, jump_threshold_, 0
	);
        
        // 4. Hawkes intensity with branching ratio
        //cuda::hawkes_intensity_kernel<<<blocks, cuda::BLOCK_SIZE>>>(
	...
	cuda::launch_hawkes_computation(
	    d_timestamps_, d_hawkes_intensity_, d_branching_ratio_, d_endogeneity_,
	    n_trades, hawkes_mu_, hawkes_phi_, hawkes_kappa_, 0
	);
        
        // 5. Poisson baseline
        //cuda::poisson_intensity_kernel<<<blocks, cuda::BLOCK_SIZE>>>(
        //    d_timestamps_, d_prices_, d_poisson_intensity_, d_poisson_surprise_,
        //    n_trades, window_size_, 60.0f  // 60 second window
        //);
	cuda::launch_poisson_computation(
             d_timestamps_, d_prices_, d_poisson_intensity_, d_poisson_surprise_,
             n_trades, window_size_, 60.0f, 0
        );
        
        // 6. Burst detection
        //cuda::burst_detection_kernel<<<blocks, cuda::BLOCK_SIZE>>>(
        //    d_hawkes_intensity_, d_poisson_surprise_, d_branching_ratio_,
        //    d_burst_flags_, d_burst_scores_,
        //    n_trades, 0.8f, 3.0f, 4.0f  // thresholds
        //);
	cuda::launch_burst_computation(
            d_hawkes_intensity_, d_poisson_surprise_, d_branching_ratio_,
            d_burst_flags_, d_burst_scores_,
            n_trades, 0.8f, 3.0f, 4.0f, 0
        );
        
        // Copy results back
        cudaMemcpy(sigma_buffer_.data(), d_sigma_, n_returns * sizeof(float), cudaMemcpyDeviceToHost);
        ...
        cudaMemcpy(bns_stats.data(), d_bns_stats_, n_returns * sizeof(float), cudaMemcpyDeviceToHost);
	...
	// Replace the problematic cudaMemcpy calls around lines 282-284 with:
        ...
        // Summary statistics
        int total_jumps = std::count(jump_flags.begin(), jump_flags.end(), true);
        int total_bursts = std::count(burst_flags.begin(), burst_flags.end(), true);
    ...
    void process_cpu(const std::vector<Trade>& trades) {
        ...
        for (size_t i = window_size_; i < return_buffer_.size() && i < trades.size(); ++i) {
            ...
            float local_vol = std::sqrt(std::max(0.0f, bv[i]));  // Fixed: no division by window_size
